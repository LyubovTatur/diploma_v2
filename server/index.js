        ...
        (err, result) => {
            if (err) {
                ...
                db.query('insert into animation values (?,?,?,?,?,?,?)',
                    [newAnimationId, req.body.login, req.body.title === '' ? 'без названия' : req.body.title, req.body.description === '' ? 'без описания' : req.body.description, 0, newDate, 1],
                    (err, result) => {
                        if (err) {
                            console.log(err)
                            //res.send('1')
    ...
    db.query(`select * from comments`,
        (err, result) => {
            if (err) {
                ...
                console.log('результ узнавания comments', result)
                res.send(result)
...
app.get('/get_avatar/:login', (req, res) => {
    console.log('логин для аватара', req.params.login)
    ...
    db.query(`select avatar from users where login = "${req.params.login}"`,
        (err, result) => {
            if (err) {
                ...
                // console.log('результ узнавания avatar',result[0].avatar)
                try {
    ...
    db.query(`select * from likes where from_login='${req.params.from_login}' and id_animation=${req.params.id_animation}`,
        (err, result) => {
            if (err) {
                ...
                console.log('результ узнавания лайка', result)
                if (result.length === 0) {
                    res.send(false)
                } else {
                    res.send(true)
    ...
    db.query(`select count(id) amount from likes where id_animation=${req.params.id_animation}`,
        (err, result) => {
            if (err) {
                ...
                console.log('результ узнавания лайков', result)
                if (result.length !== 0) {
